...
def calculate_projection_matrix(
    ...
    mn = np.zeros([len(points_2d) * 2, 1])
    m1 = np.zeros([len(points_2d) * 2, 11])
    for i in range(len(points_2d)):
        u, v, z = points_3d[i]
        x, y = points_2d[i]
        m1[i * 2] = np.array([u, v, z, 1, 0,  0,  0, 0, -x * u, -x * v, -x * z])
        mn[i * 2] = x
        m1[i * 2 + 1] = np.array([0,  0,  0,  0, u, v, z, 1, -y * u, -y * v, -y * z])
        mn[i * 2 + 1] = y
        
    M, re, ra, w = np.linalg.lstsq(m1, mn, rcond = None)
...
def projection(P: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    ...
    points_3d = np.append(points_3d, np.ones((points_3d.shape[0],1)), axis = 1) if points_3d.shape[1] == 3 else points_3d
    fprev = np.dot(P, points_3d.T)
    projected_points_2d[:, 0] = (fprev[0] / fprev[2]).T
    projected_points_2d[:, 1] = (fprev[1] / fprev[2]).T
